...
class ObjectDef:
    ...
    def call_method(self, method_name, actual_params, super_only, line_num_of_caller):
        ...
        if status == ObjectDef.STATUS_EXCEPTION or (status == ObjectDef.STATUS_RETURN and return_value is not None):
    ...
    def __execute_try_aux(self,env,return_type,code,nest_count):
        status, statement_return = self.__execute_statement(env, return_type, code[1])
        if status == ObjectDef.STATUS_EXCEPTION:
            if statement_return.type() != ObjectDef.STRING_TYPE_CONST:
                self.interpreter.error(
                    ErrorType.TYPE_ERROR,
                    "Invalid exception thrown",
                    code[0].line_num,
            ...
            if nest_count == 1:
                ...
                self.__add_locals_to_env(env, [[InterpreterBase.STRING_DEF, InterpreterBase.EXCEPTION_VARIABLE_DEF, '"'+ statement_return.value() + '"']], code[0].line_num)
                status,statement_return = self.__execute_try_aux(env,return_type,new_code,0)
            env.block_unnest()
        return status,statement_return

    def __execute_try(self, env, return_type, code):
        status, statement_return = self.__execute_try_aux(env, return_type, code,1)
        return status, statement_return
    ...
    def __evaluate_expression(self, env, expr, line_num_of_statement):
        ...
        if operator in self.binary_op_list:
            s1,operand1 = self.__evaluate_expression(env, expr[1], line_num_of_statement)
            if s1 == ObjectDef.STATUS_EXCEPTION:
                return s1,operand1
            s2,operand2 = self.__evaluate_expression(env, expr[2], line_num_of_statement)
            if s2 == ObjectDef.STATUS_EXCEPTION:
                return s2,operand2
            if (
        ...
        if operator == InterpreterBase.NEW_DEF:
            status = ObjectDef.STATUS_PROCEED
            value = self.__execute_new_aux(env, expr, line_num_of_statement)
            return status,value
